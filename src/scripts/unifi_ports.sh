#!/bin/bash
# AI Technician: port table (native/tagged VLANs, state, PoE) for one switch
# (read-only) — answers "what is plugged into the switch / port status" tickets.
# Usage: unifi_ports.sh <switch_mac>
set -u
SWITCH_MAC="${1:-}"

python3 - "$SWITCH_MAC" <<'PYEOF'
...
if not mac:
    print(json.dumps({"error": "unifi_ports requires a switch MAC target"}))
    sys.exit(1)
