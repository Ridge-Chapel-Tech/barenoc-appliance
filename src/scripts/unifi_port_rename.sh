#!/bin/bash
# AI Technician: rename a UniFi switch port.
# Usage: unifi_port_rename.sh <switch_mac> <port_idx> <new_name>
set -u
SW="$1"; PORT="$2"; NAME="$3"

python3 - "$SW" "$PORT" "$NAME" <<'PYEOF'
import sys, json, ssl, urllib.request, urllib.error

sw, port, name = sys.argv[1], sys.argv[2], sys.argv[3]
...
data, err = call("POST", f"/api/v1/unifi/ports/{sw}/{port}/rename",
                 data={"name": name}, token=tok)
if err:
