#!/bin/bash
# AI Technician: reboot a UniFi-managed device via the controller (WRITE —
# approval/autonomy governed by the Autonomy Policy, like unifi_port_config).
# Usage: unifi_restart.sh <device_mac>
set -u
DEV_MAC="${1:-}"

python3 - "$DEV_MAC" <<'PYEOF'
...
if not mac:
    print(json.dumps({"error": "unifi_restart requires a device MAC target"}))
    sys.exit(1)
...
data, err = call("POST", f"/api/v1/unifi/devices/{mac}/restart", token=tok)
if err:
