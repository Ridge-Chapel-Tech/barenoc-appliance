#!/bin/bash
# AI Technician: fetch the network/VLAN/SSID configuration from the UniFi
# controller (read-only) — answers "what vlans are on my network" tickets.
...
data, err = call("GET", "/api/v1/unifi/networks", token=tok)
if err:
