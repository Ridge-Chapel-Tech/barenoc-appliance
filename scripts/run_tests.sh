...
echo "==> api tests"
( cd src/api && python3 -m unittest test_devices test_device_agent test_device_channels test_device_layout test_devices_polish test_dashboard test_admin test_settings test_backups_setup test_alerting test_link_monitor test_starlink test_unifi_sync test_chat_juniper test_chat_shell test_updates test_tickets test_tone_filter test_network_opt test_telemetry test_wiki test_firmware test_roles test_support test_report_submit test_emailer 2>&1 | tail -3 )
